    ...
    foreach ($fields as $field):
        ...
        <div class="field-group <?php echo isset($field['class']) ? $field['class'] : ''; ?>">
            ...
            <div class="field-input">
                ...
                switch ($field['type']) {
                    case 'textfield':
                        echo getTextfield($field['param_name'], $value);
                        break;
                    case 'inwave_select':
                        echo getInwaveSelect($field['param_name'], $value, $field['data'], $field['multiple'] ? $field['multiple'] : 0, $field['class'] ? $field['class'] : '');
                        break;
                    case 'textarea':
                        echo getTextAreaField($field['param_name'], $value);
                        break;
                    case 'dropdown':
                        if (!$value)
                            $value = array();
                        echo getDropdownField($field['param_name'], $value);
                        break;
                    case 'textarea_html':
                        echo getTextAreaHtmlField($field['param_name'], $value);
                        break;
                    case 'attach_image':
                        echo getAttachImage($field['param_name'], $value);
                        break;
                    case 'iconpicker':
                    case 'iwicon':
                        echo getIwIconField($field['param_name'], $value, $field['class'] ? $field['class'] : '');
                        break;
                    case 'colorpicker':
                        echo getColorPickerField($field['param_name'], $value);
                        break;
                    case 'courses_categories':
                        echo getIwCourseCategories($field['param_name'], $value);
                        break;
                    case 'iw_range_skillbar':
                        echo getIwRangeSkillbar($field['param_name'], $value, $field['class'] ? $field['class'] : '');
                        break;
                    case 'iw_map':
                        echo getIwMapFieldHtml($field['param_name'], $field['value'], $field['class'] ? $field['class'] : '');
                        break;
                    default:
                        break;
                }
...
function getIwIconField($fname, $fvalue, $class) {
    ob_start();
    ?>
...
function getInwaveSelect($fname, $fvalue, $fdata, $fmulti, $class) {
...
if (!function_exists('inwave_getElementByTags')) {
     ...
     * @return type
     */
    function inwave_getElementByTags($tag, $content, $type = 1) {
        if ($type == 1) {
            $pattern = "/\[$tag(.*)\](.*)\[\/$tag\]/Uis";
        } elseif ($type == 2) {
            $pattern = "/\[$tag(.*)\]/Uis";
