...
/*
  ...
  Description: Includes advanced addon elements for Visual Composer (inCharity theme)
  Version: 1.1.3
  Author: Inwavethemes
...
class InwaveVC_Support {
    ...
    function loadShortCodeSettings() {
        ...
        if (isset($iw_shortcodes[$shortcode])) {
    ...
    function generate_shortcode_params() {
        ...
        /* Generate param type "iw_range_skillbar" */
        if (function_exists('vc_add_shortcode_param')) {
            vc_add_shortcode_param('iw_map', array($this, 'iw_map'));
        }
    }
    ...
    function athlete_scripts() {
        ...
        wp_enqueue_style('iw-shortcodes', plugins_url() . '/iw_composer_addons/assets/css/iw-shortcodes.css', array(), $theme_info->get('Version'));
        wp_enqueue_style('owl-carousel', plugins_url() . '/iw_composer_addons/assets/css/owl.carousel.css', array(), $theme_info->get('Version'));
        ...
        wp_enqueue_script('iw-shortcodes', plugins_url() . '/iw_composer_addons/assets/js/iw-shortcodes.js', array(), $theme_info->get('Version'), true);
        wp_enqueue_script('owl-carousel-min', plugins_url() . '/iw_composer_addons/assets/js/owl.carousel.min.js', array(), $theme_info->get('Version'), true);
        wp_enqueue_script('iw_sponsors', plugins_url() . '/iw_composer_addons/assets/js/iw_sponsors.js', array(), $theme_info->get('Version'), true);
        wp_enqueue_script('iw_custom', plugins_url() . '/iw_composer_addons/assets/js/iw_custom.js', array(), $theme_info->get('Version'), true);
    ...
    function athlete_scripts_admin() {
        ...
        wp_enqueue_script('googlemap-api', 'https://maps.googleapis.com/maps/api/js', array('jquery'), $theme_info->get('Version'));
        wp_enqueue_script('iw_map_admin', plugins_url() . '/iw_composer_addons/assets/js/iw-map-admin.js', array('jquery'), $theme_info->get('Version'), true);
        wp_enqueue_script('iw-server-location', plugins_url() . '/iw_composer_addons/assets/js/iw-server-location.js', array('jquery'), $theme_info->get('Version'), true);
    ...
    function iw_map($settings, $value) {
        $param_name = isset($settings['param_name']) ? $settings['param_name'] : '';
        $type = isset($settings['type']) ? $settings['type'] : '';
        $class = isset($settings['class']) ? $settings['class'] : '';
        $class = "wpb_vc_param_value " . $param_name . " " . $type . " " . $class;
        return getIwMapFieldHtml($param_name, $value, $class);
    }
