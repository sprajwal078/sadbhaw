...
class Inwave_Recent_Posts_Widget extends WP_Widget_Recent_Posts {
	...
	public function widget($args, $instance) {
		...
		$title = ( ! empty( $instance['title'] ) ) ? $instance['title'] : esc_html__( 'Recent Posts', 'incharity');
...
class Inwave_Recent_Comments_Widget extends WP_Widget_Recent_Comments
{
    public function widget($args, $instance)
    {
        ...
        $title = (!empty($instance['title'])) ? $instance['title'] : esc_html__('Recent Comments','incharity');
        ...
        if ($comments) {
            ...
            foreach ((array)$comments as $comment) {
                ...
                $output .= sprintf(_x('%1$s %2$s', 'widgets','incharity'),
...
class Inwave_Widget_Shortcodes extends WP_Widget {
    ...
    function __construct() {
        $widget_ops = array('classname' => 'shortcode_widget', 'description' => esc_html__('Shortcode or Arbitrary Text', 'incharity'));
        $control_ops = array('width' => 400, 'height' => 350);
        parent::__construct('shortcode-widget', esc_html__('Shortcode Widget', 'incharity'), $widget_ops, $control_ops);
    }
    ...
    function form( $instance ) {
        ...
        ?>
        <p><label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php esc_html_e('Title:', 'incharity'); ?></label>
        ...
        <p><input id="<?php echo esc_attr($this->get_field_id('filter')); ?>" name="<?php echo esc_attr($this->get_field_name('filter')); ?>" type="checkbox" <?php checked(isset($instance['filter']) ? $instance['filter'] : 0); ?> />&nbsp;<label for="<?php echo esc_attr($this->get_field_id('filter')); ?>"><?php esc_html_e('Automatically add paragraphs', 'incharity'); ?></label></p>
    <?php
    }
}
