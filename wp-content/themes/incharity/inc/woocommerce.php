...
add_action( 'after_setup_theme', 'inwave_woocommerce_support' );
function inwave_woocommerce_support() {
...
class Inwave_WC_xdays
{
    function __construct(){
        $this->settings = array(
            array(
                'name' => esc_html__('Product Newness', 'incharity'),
                'desc' => esc_html__("Display the 'New' label for how many days?", 'incharity'),
    ...
    function show_product_label() {
        ...
        if ( ( time() - ( 60 * 60 * 24 * $wc_iw_newlimitdays ) ) < $postdatestamp ) {
            echo '<span class="new-label">' . esc_html__( 'New', 'incharity' ) . '</span>';
        }
    }
}
