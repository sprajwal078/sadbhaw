...
class Inwave_Options_Machine {
    ...
    public static function optionsframework_machine($options) {
        ...
        foreach ($options as $value) {
            ...
            if (!isset($inwave_smof_data[$value['id']])) {
                $inwave_smof_data[$value['id']] = '';
            }
            ...
            /* condition start */
            if (count($inwave_smof_data) > 1 || count($data) > 1) {
                ...
                if ($value['type'] != "heading") {
                    ...
                    if (array_key_exists("fold", $value)) {
                        if (isset($inwave_smof_data[$value['fold']]) && $inwave_smof_data[$value['fold']]) {
                            $fold = "f_" . $value['fold'] . " ";
                        } else {
                ...
                switch ($value['type']) {
                    ...
                    case 'text':
                        ...
                        $t_value = stripslashes($inwave_smof_data[$value['id']]);
                    ...
                    case 'select':
                        ...
                        foreach ($value['options'] as $select_ID => $option) {
                            ...
                            $theValue = $select_ID;
                            $output .= '<option id="' . $select_ID . '" value="' . $theValue . '" ' . selected($inwave_smof_data[$value['id']], $theValue, false) . ' />' . $option . '</option>';
                        }
                        $output .= '</select></div>';
                        break;
                    ...
                    case 'textarea':
                        ...
                        $ta_value = stripslashes($inwave_smof_data[$value['id']]);
                        $output .= '<textarea class="of-input" name="' . $value['id'] . '" id="' . $value['id'] . '" cols="' . $cols . '" rows="8">' . $ta_value . '</textarea>';
                        break;

                    //radiobox option
                    case "radio":
                        $checked = (isset($inwave_smof_data[$value['id']])) ? checked($inwave_smof_data[$value['id']], $option, false) : '';
                        foreach ($value['options'] as $option => $name) {
                            $output .= '<input class="of-input of-radio" name="' . $value['id'] . '" type="radio" value="' . $option . '" ' . checked($inwave_smof_data[$value['id']], $option, false) . ' /><label class="radio">' . $name . '</label><br/>';
                        }
                        break;
                    ...
                    case 'checkbox':
                        ...
                        $output .= '<input type="checkbox" class="' . $fold . 'checkbox of-input" name="' . $value['id'] . '" id="' . $value['id'] . '" value="1" ' . checked($inwave_smof_data[$value['id']], 1, false) . ' />';
                        break;

                    //multiple checkbox option
                    case 'multicheck':
                        (isset($inwave_smof_data[$value['id']])) ? $multi_stored = $inwave_smof_data[$value['id']] : $multi_stored = "";
                    ...
                    case "color":
                        $default_color = '';
                        if (isset($value['std'])) {
                            if ($inwave_smof_data[$value['id']] != $value['std'])
                                $default_color = ' data-default-color="' . $value['std'] . '" ';
                        }
                        $output .= '<input name="' . $value['id'] . '" id="' . $value['id'] . '" class="of-color"  type="text" value="' . $inwave_smof_data[$value['id']] . '"' . $default_color . ' />';
                    ...
                    case 'typography':
                        ...
                        $typography_stored = isset($inwave_smof_data[$value['id']]) ? $inwave_smof_data[$value['id']] : $value['std'];
                    ...
                    case 'border':
                        ...
                        /* Border Width */
                        $border_stored = $inwave_smof_data[$value['id']];
                    ...
                    case 'images':
                        ...
                        $select_value = (isset($inwave_smof_data[$value['id']])) ? $inwave_smof_data[$value['id']] : '';
                    ...
                    case 'slider':
                        ...
                        $slides = $inwave_smof_data[$value['id']];
                        $count = count($slides);
                        if ($count < 2) {
                            $oldorder = 1;
                            $order = 1;
                        ...
                        $output .= '</ul>';
                        $output .= '<a href="#" class="button slide_add_button">' . esc_html__('Add New Slide', 'Avada') . '</a></div>';
                    ...
                    case 'tiles':
                        ...
                        $i = 0;
                        $select_value = isset($inwave_smof_data[$value['id']]) && !empty($inwave_smof_data[$value['id']]) ? $inwave_smof_data[$value['id']] : '';
                    ...
                    case 'backup':
                        ...
                        $backup = inwave_of_get_options(INWAVE_BACKUPS);
                        ...
                        $output .= '<p><strong>' . esc_html__('Last Backup : ', 'incharity') . '<span class="backup-log">' . $log . '</span></strong></p></div>' . "\n";
                    ...
                    case 'transfer':
                        $instructions = $value['desc'];
                        self::sanitize($inwave_smof_data);

                        if (defined('JSON_UNESCAPED_SLASHES')) {
                            $output .= '<textarea id="export_data" rows="8">' . json_encode($inwave_smof_data, JSON_UNESCAPED_SLASHES) . '</textarea>' . "\n";
                        } else {
                            $output .= '<textarea id="export_data" rows="8">' . json_encode($inwave_smof_data, 64) . '</textarea>' . "\n";
                        }
                    ...
                    case 'select_google_font':
                        ...
                        foreach ($value['options'] as $select_key => $option) {
                            $output .= '<option value="' . $select_key . '" ' . selected((isset($inwave_smof_data[$value['id']])) ? $inwave_smof_data[$value['id']] : "", $option, false) . ' />' . $option . '</option>';
                        }
                        $output .= '</select></div>';
                    ...
                    case 'sliderui':
                        ...
                        $s_val = stripslashes($inwave_smof_data[$value['id']]);
                    ...
                    case 'switch':
                        ...
                        //Get selected
                        if ($inwave_smof_data[$value['id']] == 1) {
                            $cb_enabled = ' selected';
                            $cb_disabled = '';
                        } else {
                            $cb_enabled = '';
                        ...
                        $output .= '<input type="hidden" class="' . $fold . 'checkbox of-input" name="' . $value['id'] . '" id="' . $value['id'] . '" value="0"/>';
                        $output .= '<input type="checkbox" id="' . $value['id'] . '" class="' . $fold . 'checkbox of-input main_checkbox" name="' . $value['id'] . '"  value="1" ' . checked($inwave_smof_data[$value['id']], 1, false) . ' />';
                    ...
                    case "media":
                        ...
                        $u_val = '';
                        if (isset($inwave_smof_data[$value['id']]) && $inwave_smof_data[$value['id']]) {
                            $u_val = stripslashes($inwave_smof_data[$value['id']]);
                        }
    ...
    public static function optionsframework_media_uploader_function($id, $std, $mod) {
        ...
        $uploader = '';
        $upload = isset($inwave_smof_data[$id]) ? $inwave_smof_data[$id] : '';
        $hide = '';
    ...
    public static function optionsframework_slider_function($id, $std, $oldorder, $order) {
        ...
        $slider = '';
        $slide = array();
        $slide = isset($inwave_smof_data[$id]) ? $inwave_smof_data[$id] : '';
