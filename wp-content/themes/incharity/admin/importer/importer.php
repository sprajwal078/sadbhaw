...
function inwave_importer()
{
    ...
    include WP_PLUGIN_DIR.'/iw_composer_addons/inc/wordpress-importer.php';
    ...
    switch($step){
        case 'prepare':
            ...
            foreach ($iw_post_types as $post_type) {
                switch ($post_type) {
                    ...
                    case 'portfolio':
                        ...
                        break;
					case 'infunding':
						$importer['base']->allow_post_types[] = 'infunding';
                        $importer['base']->allow_term_types[] = 'infunding_category';
                        $importer['base']->allow_term_types[] = 'infunding_tag';
                        array_push($importer['steps'], 'infunding');
                        $importer['base']->step_total += $among;
                        break;
                    case 'menu':
            ...
            $data = json_decode($theme_options_txt, true);  /* decode theme options data */
        ...
        case 'portfolio':
            ...
            break;
		case 'infunding':
            inwave_import_infunding();
            array_shift($importer['steps']);
            $importer['base']->step_done += $among;
            $message = 'Imported Crowdfunding successfully';
            break;
        ...
        case 'finish':
            ...
            $wpdb->query($wpdb->prepare("update wp_postmeta set meta_value = replace(meta_value,'http://inwavethemes.com/wordpress/incharity/wp-content','%s')", content_url()));

            $importer['base']->import_end();
            $importer['base']->step_done = $importer['base']->step_total;
            if(!$importer['base']->error_msg) {
                $message = '<b class="grey-color">Cheers! The demo data has been imported successfully! Please reload this page to finish!</b>';
            }else{
                $message = '<b class="grey-color">Data import completed!</b><br />'.'<div>'.$importer['base']->error_msg.'</div>';
            }
            $code ='completed';
            break;
        default:
...
function inwave_get_import_data_types(){
    ...
    if (class_exists('inFundingUtility')) {
        $data_types['post-type']['infunding'] = 'Crowdfunding';
    }
	...
	if ( defined( 'WPCF7_VERSION' ) ) {
		$data_types['post-type']['wpcf7_contact_form'] = 'Contact Form 7';
...
function inwave_import_pages(){
    ...
    $blogpage = get_page_by_title('Blog page');
    if ($homepage->ID) {
...
function inwave_import_sliders(){
    ...
    if (class_exists('RevSlider')) {
        ...
        // import slider 1
        $_FILES["import_file"]["tmp_name"] = get_template_directory() . '/admin/importer/data/slider-2.zip';
        ob_start();
        $revapi->importSliderFromPost();
        ob_end_clean();

        // import slider 2
        $_FILES["import_file"]["tmp_name"] = get_template_directory() . '/admin/importer/data/slider-3.zip';
        ob_start();
        $revapi->importSliderFromPost();
        ob_end_clean();

        // import slider 3
        $_FILES["import_file"]["tmp_name"] = get_template_directory() . '/admin/importer/data/slider-4.zip';
        ob_start();
        $revapi->importSliderFromPost();
        ob_end_clean();
    }
...
function inwave_import_infunding(){
	// import infunding plugin code
	return true;
}
